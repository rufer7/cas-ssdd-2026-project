// Thin wrapper around the Auth0 SPA client.
//
// Initialises the client (with the backend API audience so getTokenSilently
// returns an access token the resource server accepts), handles the redirect
// callback, wires the apiClient token provider, and exposes role helpers.
...
export async function initAuth() {
    ...
    configureApiClient({ getToken: () => client.getTokenSilently() });
    return client;
}
